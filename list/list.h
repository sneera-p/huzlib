...
#define list_init(node) typecheck_ptr(    \
...
extern void list_rotate_after(struct list_node *entry);
extern void list_rotate_before(struct list_node *entry);
extern void list_reverse(struct list_node *head);
extern void list_sort(struct list_node *head, int8_t (*cmp)(struct list_node *, struct list_node *));
...
#define list_first_entry(head, type, member) containerof((head)->next, type, member)
#define list_last_entry(head, type, member)  containerof((head)->prev, type, member)
#define list_next_entry(entr, type, member)  containerof((entr)->member.next, type, member)
#define list_prev_entry(entr, type, member)  containerof((entr)->member.prev, type, member)
...
static inline void __list_add_batch(struct list_node *new_head, struct list_node *new_tail, struct list_node *prev, struct list_node *next)
{
   ...
   assert(list_contains(new_head, new_tail) && list_contains(prev, next));
...
inline void list_rotate_after(struct list_node *entry)
{
   assert(entry);
   if (!(list_is_empty(entry) || list_is_singular(entry)))
      __list_swap_adj(entry->prev, entry, entry->next, entry->next->next);
}

inline void list_rotate_before(struct list_node *entry)
{
   assert(entry);
   if (!(list_is_empty(entry) || list_is_singular(entry)))
      __list_swap_adj(entry->prev->prev, entry->prev, entry, entry->next);
}
